...
var
  ...
  begin
    ...
    begin
      ...
      ExecSQL;
      open;
      TIWUserSession(WebApplication.Data).email := edtemail2.text;
      validation := FieldByName('validacao').AsString;
      close;
    end;
    ...
    begin
      ...
      begin
      validation := 'validado';
      data := timetostr(now) + ' - ' + datetostr(date);
        ...
        validation + '", validation_date = "' + data + '" WHERE email="' + (TIWUserSession(WebApplication.Data).email)
        + '";';
      ExecSQL;
      end;
      ...
      begin
        // salvando no BD, como a planilha problema já será atualizada no inicio deve-se apenas atualizar o campo Cri_Order
        SQL.Text := 'UPDATE user SET validacao ="' +
        validation + '" WHERE email="' + (TIWUserSession(WebApplication.Data).email)
        + '";';
        ExecSQL;
      end;
      ...
      begin
        // salvando no BD, como a planilha problema já será atualizada no inicio deve-se apenas atualizar o campo Cri_Order
        SQL.Text := 'UPDATE user_experimento SET validacao ="' +
        validation + '" WHERE email="' + (TIWUserSession(WebApplication.Data).email)
        + '";';
        ExecSQL;
      end;
      ...
      begin
        // salvando no BD, como a planilha problema já será atualizada no inicio deve-se apenas atualizar o campo Cri_Order
        SQL.Text := 'UPDATE usuario SET validacao ="' +
        validation + '" WHERE email="' + (TIWUserSession(WebApplication.Data).email)
        + '";';
        ExecSQL;
      end;
...
rgnlogin.visible := true;
rgnvalidation.Visible := false;
rgnvalidation.Left := rgnlogin.Left;
rgnvalidation.Top := rgnlogin.Top;
edtemail2.Text := '';
edtcode.Text := '';
iwlabel9.Visible := false;
iwlabel8.visible := false;
...
var
  ...
  i, a: integer;
  H: boolean;
  validation, id_user: string;
begin
  email := edtemail2.Text;
  with UserSession.ZQuery1 do
    begin

      SQL.Text := 'SELECT validacao,id FROM user WHERE email ="' + (edtemail2.Text)
        + '"';
      ExecSQL;
      open;
      validation := FieldByName('validacao').AsString;
  ...
  iwtext4.lines[6] := iwtext4.Lines[6] + ' ' + validation;
